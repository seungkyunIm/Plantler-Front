        ...
        axios.post(`${process.env.REACT_APP_BACK_HOST_URL}/boarddetail/${board_no}`, {}, { headers: {'Authorization': cookie.token} })
            .then(res=> {
                if(res.data.state) {
                    ...
                    console.log("khboard" + " 데이터 상태확인용 ( " + res.data.result.freeboard + " ) ");
                    setComments(res.data.result.comments); // 댓글 목록 가져옴
                } else {
                    console.log("실패" + res.data.result);
                    setError(res.data.result);
                        ...
                        <h1 className="sub_title">식물 무료 나눔</h1>
                            <table className="detail_table">
                                <thead>
                                    <tr>
                                    <td>
                                        <span>[{freeBoardDetail?.category_name}] </span><span>{freeBoardDetail?.board_title}</span> 
                                        <p>{freeBoardDetail?.user_nick} | {freeBoardDetail?.board_regdate}</p>
                                        <p className="f_right">조회수 {freeBoardDetail?.board_count} | 좋아요 {board_like}</p>                      
                                    </td>
                                    </tr>
                                </thead>
                                <tbody>
                                    ...
                                    <td>
                                        ...
                                        <p style={{textAlign: 'center'}}><img src={freeBoardDetail?.file_no == 0 ? main.main_bg : getImgUrl(freeBoardDetail?.file_no)} alt='img_01' /></p>
                                    </td>
                                    </tr>
                                </tbody>
                            </table>
                    ...
                    <div className="col-md-12 ol-sm-12" style={{backgroundColor: '#f3f3f3'}}>
                       {/* 댓글 창 */}
                        ...
                        <div style={{padding: '20px 20px'}}>
                            {comments?.map((v, i) => <FreeboardComment v={v} key={i} f={getData}/>)}
                        </div>
